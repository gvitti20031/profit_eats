...
# Define symbolic variables for gradient calculations
p1, d1, t1, m1, e1 = sp.symbols('p d t m e')
P = (50.82 * p1 / d1) - ((e1 * t1) / m1)
...
def calculate():
    try:
        ...
        # Remove gas price input from index.html and use the mapped value from the CSV
    ...
    # Define calculations
    def rev(duration, price):
        return 60 * price / duration

    def cost(duration, expense, mileage, price, travel):
        return expense * travel / mileage + rev(duration, price) * 0.153

    def profit(duration, expense, mileage, price, travel):
        return rev(duration, price) - cost(duration, expense, mileage, price, travel)

    try:
        profit_val = round(profit(d, e, m, p, t), 2)

        # Calculate gradients with variable substitution for sensitivity analysis
        current_p_grad = dP_dp.subs({p1: p, d1: d, t1: t, m1: m, e1: e})
        current_d_grad = dP_dd.subs({p1: p, d1: d, t1: t, m1: m, e1: e})
        current_t_grad = dP_dt.subs({p1: p, d1: d, t1: t, m1: m, e1: e})
        current_m_grad = dP_dm.subs({p1: p, d1: d, t1: t, m1: m, e1: e})
        current_e_grad = dP_de.subs({p1: p, d1: d, t1: t, m1: m, e1: e})

        p_grad_num = round(float(current_p_grad), 2)
        d_grad_num = round(float(current_d_grad), 2)
        t_grad_num = round(float(current_t_grad), 2)
        m_grad_num = round(float(current_m_grad), 2)
        e_grad_num = round(float(current_e_grad), 2)
